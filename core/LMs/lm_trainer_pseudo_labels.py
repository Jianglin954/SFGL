...
class LM_PL_Trainer():
    def __init__(self, cfg):
        ...
        # _ablation_each_class_1_label  _#_train_LMs
        self.orign_data_split = "_#_train_LMs_pls" if cfg.lm.train.GSL_data_split else ""
        # self.orign_data_split = "_#_train_LMs_pls_iter_GPT" if cfg.lm.train.GSL_data_split else ""
        # self.output_dir = f'output/{self.dataset_name}{self.use_gpt_str}{self.orign_data_split}/{self.model_name}-seed{self.seed}'
        # self.ckpt_dir = f'prt_lm/{self.dataset_name}{self.use_gpt_str}{self.orign_data_split}/{self.model_name}-seed{self.seed}'

        #self.output_dir = f'output/{self.dataset_name}{self.use_gpt_str}{self.orign_data_split}/{self.model_name}-seed{self.seed}-labels{self.ratio}-lr{self.lr}'
        #self.ckpt_dir = f'prt_lm/{self.dataset_name}{self.use_gpt_str}{self.orign_data_split}/{self.model_name}-seed{self.seed}-labels{self.ratio}-lr{self.lr}'
        ### print("------------manhattan---------")
        ...
        #self.ckpt_dir = f'prt_lm/{self.dataset_name}{self.use_gpt_str}{self.orign_data_split}/{self.model_name}-seed{self.seed}-ratio{self.ratio}'

        # Preprocess data
        data, num_classes, text = load_data(
            dataset=self.dataset_name, use_text=True, use_gpt=cfg.lm.train.use_gpt, seed=self.seed, ratio=self.ratio)
        self.data = data
...
class LM_Text_Encoder_Trainer():
    ...
    @time_logger
    def train(self):
        print("Text encoder, no training, only inference...")
        '''        
        # Define training parameters
        eq_batch_size = self.batch_size * 4
        train_steps = self.num_nodes // eq_batch_size + 1
